...
tapped: the first of `data["url"]`, `data["deep_link"]`, `data["link"]` or `data["launch_url"]`
that carries a value. The SDK opens it itself, in every app state; it is exposed so a host that
would rather route the link its own way can read the same value. Action-button taps do not open
it.
